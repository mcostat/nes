...
from experiment.models import Component, QuestionnaireResponse
...
class ABCSearchEngineTest(TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            lime_survey.activate_tokens(self.test_sid)
            lime_survey.activate_survey(self.test_sid)

            # Adiciona participante e obtem o token
            result_token = lime_survey.add_participant(self.test_sid)
    ...
    def tearDown(self) -> None:
        super().tearDown()
        Questionnaires().delete_survey(self.test_sid)
...
class SurveyTest(TestCase):
    ...
    def setUp(self):
        ...
        self.response = UtilTests.create_response_survey(self.user, self.patient, self.survey, token_id=1)

    @patch("survey.abc_search_engine.Server")
    def test_survey_list(self, mock_server):
    ...
    @patch("survey.abc_search_engine.Server")
    def test_survey_create(self, mock_server):
        mock_server.return_value.get_session_key.return_value = "vz224sb7jzkvh8i4kpx8fxbcxd67meht"
        ...
        mock_server.return_value.get_summary.return_value = 2
    ...
    @patch("survey.abc_search_engine.Server")
    def test_survey_update(self, mock_server):
        mock_server.return_value.get_session_key.return_value = "vz224sb7jzkvh8i4kpx8fxbcxd67meht"
        # TODO (NES-981): probably this errors are due to running without LIMESURVEY=remote.
        #  Run with it.
        mock_server.return_value.get_survey_properties.return_value = {"status": "Error: Invalid survey ID"}
        mock_server.return_value.get_language_properties.return_value = {"status": "Error: Invalid survey ID"}
        ...
        response = self.client.post(reverse("survey_edit", args=(self.survey.pk,)), self.data, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    @patch("survey.abc_search_engine.Server")
    def test_survey_view(self, mock_server):
        self._set_mocks(mock_server)
        response = self.client.get(reverse("survey_view", args=(self.survey.pk,)))
        self.assertEqual(response.status_code, 200)

    @patch("survey.abc_search_engine.Server")
    def test_get_survey_view_display_update_acquisitiondate_buttons(self, mock_server):
        self._set_mocks(mock_server)
        # token_id=2 from mocks
        self.create_experiment_questionnaire_response(self.patient, 2, self.survey)

        response = self.client.get(reverse("survey_view", args=(self.survey.pk,)))
        self.assertContains(response, "Atualizar data de preenchimento do LimeSurvey", 2)

    @patch("survey.abc_search_engine.Server")
    def test_update_acquisitiondate_from_limesurvey(self, mock_server):
        self._set_mocks(mock_server)
        ...
        self.client.get(reverse("update_survey_acquisitiondate", args=(self.survey.pk,)))

        entrance_questionnaire_response = PatientQuestionnaireResponse.objects.get(token_id=1)
        experiment_questionnaire_response = QuestionnaireResponse.objects.get(token_id=2)
    ...
    @patch("survey.abc_search_engine.Server")
    def test_update_acquisitiondate_from_limesurvey_with_date_in_wrong_format_fails_silently(self, mock_server):
        self._set_mocks(mock_server)
    ...
    @patch("survey.abc_search_engine.Server")
    def test_get_update_survey_acquisitiondate_view_redirects_to_survey_view(self, mock_server):
        self._set_mocks(mock_server)

        response = self.client.get(reverse("update_survey_acquisitiondate", args=(self.survey.pk,)))
        survey_view_url = reverse("survey_view", args=(self.survey.pk,))
        self.assertRedirects(response, survey_view_url, 302)

    @patch("survey.abc_search_engine.Server")
    def test_update_acquisitiondate_from_limesurvey_returns_responses_updated1(self, mock_server):
        ...
        tokens = mock_server.return_value.list_participants.return_value
        ls_responses = b64decode(mock_server.return_value.export_responses.return_value).decode()
    ...
    @patch("survey.abc_search_engine.Server")
    def test_update_acquisitiondate_from_limesurvey_returns_responses_updated2(self, mock_server):
        ...
        questionnaire_response_2 = UtilTests.create_response_survey(self.user, patient2, self.survey, token_id=2)
    ...
    @patch("survey.abc_search_engine.Server")
    def test_get_update_survey_acquisitiondate_view_redirects_to_survey_view_with_right_message1(self, mock_server):
        """No responses were updated"""
    ...
    @patch("survey.abc_search_engine.Server")
    def test_get_update_survey_acquisitiondate_view_redirects_to_survey_view_with_right_message2(self, mock_server):
    ...
    @patch("survey.abc_search_engine.Server")
    def test_survey_without_pt_title_gets_listed_with_en_title_instead_but_remains_without_pt_title(self, mock_server):
        mock_server.return_value.get_session_key.return_value = "vz224sb7jzkvh8i4kpx8fxbcxd67meht"
    ...
    @override_settings(LANGUAGE_CODE="en")
    @patch("survey.abc_search_engine.Server")
    def test_survey_without_en_title_gets_listed_with_pt_title_instead_but_remains_without_en_title(self, mock_server):
        mock_server.return_value.get_session_key.return_value = "vz224sb7jzkvh8i4kpx8fxbcxd67meht"
    ...
    def test_survey_without_pt_or_en_title_returns_default_language_title_to_be_listed_but_does_not_save(
        ...
        mock_server.return_value.get_survey_properties.return_value = {"active": "N"}
    ...
    def test_surveys_list_get_updated(self, mock_server):
        mock_server.return_value.get_session_key.return_value = "vz224sb7jzkvh8i4kpx8fxbcxd67meht"
        mock_server.return_value.add_survey.return_value = 212121
        mock_server.return_value.get_language_properties.side_effect = [
    ...
    @staticmethod
    def _set_mocks(mock_server):
        mock_server.return_value.get_session_key.return_value = "abc"
        mock_server.return_value.get_survey_properties.return_value = {
            "language": "en",
            "additional_languages": "",
        }
        mock_server.return_value.get_participant_properties.return_value = {"completed": "2020-09-25 09:19"}
        mock_server.return_value.get_language_properties.return_value = {"surveyls_title": "Survey title"}
        mock_server.return_value.list_participants.return_value = [
            {
                "tid": 1,
